...
const ignoredFiles = ["src\\docs\\md\\404.md"];
 ...
 * Splits it into sub-sections based on criteria.
 */
function processMdForSearch(content: string): ProcessedMd {
  ...
  // console.log("processMdForSearch", { sections, checksum });
...
async function generateEmbeddings() {
  ...
  const markdownFiles = (await walk("src/docs/md"))
  ...
  for (const markdownFile of markdownFiles) {
    ...
    try {
      ...
      for (const section of sections) {
        ...
        try {
          ...
          if (embeddingResponse.status !== 200) {
            console.log(`Open AI Error: ${embeddingResponse.status}`);
      ...
      // Set page checksum so that we nkow this page was stored successfully
